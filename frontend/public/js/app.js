...
/* nedb returns _id, normalise everything to use .id so the rest of the code is consistent */
...
async function loadAll() {
  try {
    const raw = await apiFetch('/invoices');
    allInvoices = raw.map(normalise);
    renderSidebar();
  } catch (e) {
...
async function saveInvoice() {
  ...
  try {
    ...
    toast('Save failed — check server', 'error');
    return null;
  } finally {
...
/* FIX: Export PDF — save first (only if not already saved), then export from current form data
   Never calls saveInvoice() in a loop — uses the saved id or the current form directly */
async function exportPDF() {
  ...
  try {
    // If already saved (has editingId), just update it. If brand new, save once.
    const saved = await saveInvoice();
    if (!saved && !editingId) { toast('Please fill in invoice details first', 'error'); return; }

    // Fetch the saved invoice fresh from server (has correct items from DB)
    const id = editingId || (saved && saved.id);
    if (!id) { toast('Could not get invoice ID', 'error'); return; }
